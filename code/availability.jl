...
function availabilityPlot(algorithmns,
    ...
    # Create Gurobi environment for following optimizations
    env = Gurobi.Env()
    # Create a blank 2-d list to record availabilities
    ...
    # These two lists store different scenarios and corresponding possibilities derived from different topologies
    scenarios_all = []
    scenario_probs_all = []
    # Iterate through different topologies(We only use one topology at one time now)
    for t in 1:length(topologies)
        topology = topologies[t]
        # Read topologies(directed graph with weighted arcs G=(V,A))from data folder, topologies contain:
        # links: arcs a in A
        # capacity: {u(a)|a in A}
        # link_probs: probabilities of failure of links, {p_{failure}(a)|a in A}, however this value is not used in the code below
        # nodes: vertices v in V
        links, capacity, link_probs, nodes = readTopology(topology)
        # These two lists store different scenarios and corresponding possibilities derived from a single topology
        scenarios_all_top = []
        scenario_probs_top = []
        # Sample several scenarios with different link failure probabilities by pre-defined weibull distributions
        for i in 1:iterations
            # Probabilities of failure of links, {p_{failure}(a)|a in A}, generated by weibull distributions
            link_probs = weibullProbs(length(links), shape=.8, scale=weibull_scale)
            # Iterate through different combinations of link working status. This process compute all scenarios with probabilities above the threshold #cutoff
            ...
            # Stop printing here because it is too noisy
            # println(probs)
        end
...
# The following notation are left by the original author. I guess they can generate some plots present in the paper.
